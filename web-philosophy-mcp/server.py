#!/usr/bin/env python3
"""
MCP Server: Web Philosophy - UniversInside
============================================
Servidor MCP que fuerza la filosofía de programación modular
para desarrollo web (HTML, CSS, JS vanilla) con Atomic Design.
...
PHILOSOPHY = {
    ...
    "levels": {
        "atom": "Elemento básico indivisible, hace UNA sola cosa → atoms/*",
        "molecule": "Combina átomos en un grupo funcional → molecules/*",
        "organism": "Sección compleja, combina moléculas → organisms/*",
        "template": "Layout de página, distribución de organismos → templates/*",
        "page": "Instancia concreta de un template con datos reales → pages/*"
    },
    "naming": {
        "web": {
            "atom": r".*/atoms?/.*",
            "molecule": r".*/molecules?/.*",
            "organism": r".*/organisms?/.*",
    ...
    "code_smells": {
        "web": [
            ...
            (r'on\w+\s*=\s*["\']', "Evita event handlers inline. Usa addEventListener en JS."),
            (r'document\.write\s*\(', "Evita document.write. Usa manipulación del DOM."),
        ],
        "css": [
            (r'!important', "Evita !important. Refactoriza la especificidad de los selectores."),
        ],
        "html": [
            (r'style\s*=\s*["\']', "Evita estilos inline. Usa clases CSS."),
            (r'on\w+\s*=\s*["\']', "Evita event handlers inline. Usa addEventListener."),
        ],
        "js": [
            (r'^var\s+\w+', "Usa const o let en vez de var."),
        ]
    }
}
...
4. Mapea cada archivo al nivel correcto (atom/molecule/organism/template/page)
...
async def step0_criterios(tarea: str, reformulacion: str, criterios: list, confirmado_por_usuario: bool, project_path: str = None) -> str:
    ...
    patrones_criterio_codigo = [
        ...
        r'=\s*\d+',  # "= 0", "= 5"
        r'\b(display|flex|grid|margin|padding|font-size)\s*:',  # propiedades CSS
        r'\b(querySelector|getElementById|addEventListener|innerHTML)\b',  # DOM API
        r'\b(var|let|const)\s+\w+\s*=',  # declaraciones de variables
        r'\b(className|classList|style\.)\b',  # manipulación de estilo/clase
...
def detectar_duplicacion(archivos: list, project_path: Path, language: str) -> dict:
    ...
    if language == "web":
        patrones_sospechosos = [
            (r'style\s*=\s*["\'][^"\']{20,}', "Estilos inline extensos"),
            (r'(#[0-9a-fA-F]{3,8})', "Colores hardcodeados"),
            (r'!important', "!important"),
            (r'function\s+(handle|create|render|update)\w+', "Funciones con prefijo común"),
            (r'document\.(querySelector|getElementById)\s*\(', "Queries al DOM repetidas"),
            (r'<div\s+class="[^"]*"', "Estructuras HTML con clases"),
        ]
    else:
        patrones_sospechosos = [
...
async def step3_buscar(search_term: str, project_path: str, content_pattern: str = None,
    ...
    if rg_available:
        ...
        try:
            ...
            result = subprocess.run(
                ["rg", "--files", "--glob", "!.git", "--glob", "!__pycache__", "--glob", "!addons", "--glob", "!node_modules", "--glob", "!dist", "--glob", "!build"] + glob_args,
        ...
        if content_pattern:
            try:
                ...
                for ext in extensions:
                    glob_args.extend(["--glob", f"*{ext}"])
                result = subprocess.run(
                    ["rg", "-l", "-i", "--glob", "!.git", "--glob", "!__pycache__", "--glob", "!addons", "--glob", "!node_modules", "--glob", "!dist", "--glob", "!build"] + glob_args + [content_pattern],
                    capture_output=True, text=True, cwd=str(path), timeout=30
                )
                for line in result.stdout.splitlines():
    ...
    language = SESSION_STATE.get("current_language", "web")
...
NIVEL_KEYWORDS = {
    "atom": ["una sola cosa", "atómico", "mínimo", "único", "botón", "input", "label", "elemento básico", "indivisible"],
    "molecule": ["combina átomos", "agrupa", "campo de formulario", "search bar", "card simple", "combina elementos"],
    "organism": ["sección", "header", "footer", "sidebar", "navigation", "conjunto", "grid", "lista compleja", "formulario completo"],
    "template": ["layout", "distribución", "estructura de página", "wireframe", "disposición"],
    "page": ["instancia", "página concreta", "con datos reales", "home", "about", "contacto"]
}

# Palabras que son INCOMPATIBLES con cada nivel (detectar errores obvios)
NIVEL_INCOMPATIBLE = {
    "atom": ["coordina", "orquesta", "secciones", "páginas", "layout"],
    "molecule": ["coordina secciones", "orquesta organismos", "layout de página"],
    "organism": ["página concreta", "instancia con datos", "atómico", "indivisible"],
    "template": ["atómico", "una sola cosa mínima", "elemento básico", "botón"],
    "page": []
}
...
def get_suggested_filename(nivel: str, current_filename: str, language: str) -> str:
    """Genera el nombre de archivo sugerido según nomenclatura Atomic Design"""
    import os
...
COMPORTAMIENTO POR NIVEL:
   • Atom       → Elemento básico, hace UNA sola cosa
   • Molecule   → Combina átomos en grupo funcional
   • Organism   → Sección compleja, combina moléculas
   • Template   → Layout de página, distribución
   • Page       → Instancia con datos reales
...
NOMENCLATURA CORRECTA:
   • Atom       → atoms/*.html, atoms/*.css, atoms/*.js
   • Molecule   → molecules/*.html, molecules/*.css, molecules/*.js
   • Organism   → organisms/*.html, organisms/*.css, organisms/*.js
   • Template   → templates/*.html, templates/*.css, templates/*.js
   • Page       → pages/*.html, pages/*.css, pages/*.js
...
def extraer_propiedades_referencia(content: str, lines: list, must_document: list, language: str) -> dict:
    ...
    for prop in must_document:
        ...
        # Patrones de búsqueda para web (JS, CSS, HTML)
        patterns = [
...
async def step6_verificar_dependencias(project_path: str, dependencies: list = None, references: list = None,
    ...
    for dep in dependencies:
        ...
        # JS: function nombre(params) o export function nombre(params) o async
...
async def step8_validate(code: str = None, filename: str = None, file_path: str = None,
    ...
    # ===============================================================
    # RAMA ESPECIAL: Archivos CSS
    # ===============================================================
    is_css = filename.endswith('.css')

    if is_css:
        # 1. Detectar colores hardcodeados sin CSS variables
        hardcoded_colors = []
        for i, line in enumerate(lines, 1):
            stripped = line.strip()
            if stripped.startswith('//') or stripped.startswith('/*'):
                continue
            # Colores hex o rgb/rgba que NO están dentro de var() o como valor de variable CSS
            if re.search(r':\s*(#[0-9a-fA-F]{3,8}|rgb[a]?\s*\([^)]+\))', stripped):
                if 'var(' not in stripped and not stripped.strip().startswith('--'):
                    hardcoded_colors.append((i, stripped))

        if len(hardcoded_colors) > 3:
            warnings.append(f"⚠️ {len(hardcoded_colors)} colores hardcodeados sin variables CSS. Usa var(--color-nombre).")
            for line_num, line_content in hardcoded_colors[:3]:
                warnings.append(f"   Línea {line_num}: {line_content[:80]}")

        # 2. Detectar !important
        important_count = sum(1 for line in lines if '!important' in line)
        if important_count > 0:
            warnings.append(f"⚠️ {important_count} usos de !important. Refactoriza la especificidad de selectores.")

        # 3. Detectar selectores demasiado específicos (>3 niveles)
        deep_selectors = []
        for i, line in enumerate(lines, 1):
            if '{' in line:
                selector = line.split('{')[0].strip()
                # Contar niveles de especificidad (espacios, >, +, ~)
                parts = re.split(r'\s+|>|\+|~', selector)
                parts = [p for p in parts if p and not p.startswith('@') and not p.startswith('/')]
                if len(parts) > 3:
                    deep_selectors.append((i, selector))

        if deep_selectors:
            warnings.append(f"⚠️ {len(deep_selectors)} selectores con >3 niveles de profundidad.")
            for line_num, sel in deep_selectors[:3]:
                warnings.append(f"   Línea {line_num}: {sel[:80]}")

        # 4. DRY: Detectar clases CSS duplicadas con mismas propiedades
        css_blocks = {}
        ...
        for line in lines:
            ...
            if '{' in stripped and not stripped.startswith('/*'):
                if current_selector and current_props:
                    css_blocks[current_selector] = sorted(current_props)
                ...
                current_selector = None
                current_props = []
            elif current_selector and ':' in stripped and not stripped.startswith('/*'):
                current_props.append(stripped.rstrip(';').strip())

        # Buscar bloques con propiedades idénticas
        props_to_selectors = {}
        for selector, props in css_blocks.items():
            key = tuple(props)
            if key and len(key) >= 2:
                props_to_selectors.setdefault(key, []).append(selector)

        for props_key, selectors in props_to_selectors.items():
            if len(selectors) >= 2:
                issues.append(
                    f"❌ DRY: Selectores {', '.join(selectors[:3])} tienen propiedades idénticas. "
                    f"Crea una clase reutilizable."
                )

        # Construir respuesta CSS
        response = f"""
╔══════════════════════════════════════════════════════════════════╗
║  PASO 8/9: VALIDACIÓN (CSS)                                      ║
╚══════════════════════════════════════════════════════════════════╝

📄 ARCHIVO: {filename}
🔧 TIPO: Hoja de estilos CSS
📏 LÍNEAS: {len(lines)}
📦 Selectores: {len(css_blocks)}
...
"""
        ...
        if not issues and not warnings:
            ...
            response += "✅ CSS APROBADO\n\n"
            response += "La hoja de estilos cumple con DRY y la filosofía modular.\n\n"
...
"""
            ...
            if usuario_confirmo_warnings:
                SESSION_STATE["step_8"] = True
                response += "✅ CSS APROBADO (usuario confirmó ignorar advertencias)\n\n"
                response += """➡️ PASO 9 (OBLIGATORIO): Usa philosophy_q9_documentar
...
"""
                ...
                response += """⚠️ CSS CON ADVERTENCIAS - REQUIERE DECISIÓN DEL USUARIO
...
"""
    ...
    # ===============================================================
    # RAMA ESPECIAL: Archivos HTML
    # ===============================================================
    is_html = filename.endswith('.html') or filename.endswith('.htm')

    if is_html:
        # 1. Detectar estilos inline
        inline_styles = []
        for i, line in enumerate(lines, 1):
            if re.search(r'style\s*=\s*["\']', line):
                inline_styles.append((i, line.strip()))

        if inline_styles:
            issues.append(f"❌ {len(inline_styles)} estilos inline. Usa clases CSS reutilizables.")
            for line_num, content in inline_styles[:3]:
                issues.append(f"   Línea {line_num}: {content[:80]}")

        # 2. Detectar event handlers inline
        inline_handlers = []
        for i, line in enumerate(lines, 1):
            if re.search(r'\bon\w+\s*=\s*["\']', line):
                inline_handlers.append((i, line.strip()))

        if inline_handlers:
            warnings.append(f"⚠️ {len(inline_handlers)} event handlers inline. Usa addEventListener en JS.")

        # 3. Detectar div soup (ratio divs vs semantic)
        div_count = sum(1 for line in lines if re.search(r'<div[\s>]', line, re.IGNORECASE))
        ...
        if div_count > 10 and semantic_count == 0:
            issues.append(f"❌ Divitis: {div_count} <div> y 0 etiquetas semánticas. Usa <header>, <nav>, <main>, <section>, <article>, <footer>.")
        elif div_count > 5 and div_count > semantic_count * 3:
            warnings.append(f"⚠️ {div_count} <div> vs {semantic_count} semánticas. Considera usar más etiquetas semánticas.")

        # 4. Detectar imágenes sin alt
        img_no_alt = []
        for i, line in enumerate(lines, 1):
        ...
        if img_no_alt:
            warnings.append(f"⚠️ {len(img_no_alt)} imágenes sin atributo alt (accesibilidad).")

        # 5. DRY visual: Detectar estructuras HTML repetidas
        html_structures = {}
        for i, line in enumerate(lines):
            stripped = line.strip()
            if len(stripped) > 40 and stripped.startswith('<') and not stripped.startswith('<!'):
                html_structures[stripped] = html_structures.get(stripped, 0) + 1

        repeated = {k: v for k, v in html_structures.items() if v >= 3}
        if repeated:
            warnings.append(f"⚠️ DRY visual: {len(repeated)} estructuras HTML repetidas 3+ veces. Crea un componente reutilizable.")

        # Construir respuesta HTML
        response = f"""
╔══════════════════════════════════════════════════════════════════╗
║  PASO 8/9: VALIDACIÓN (HTML)                                     ║
╚══════════════════════════════════════════════════════════════════╝

📄 ARCHIVO: {filename}
🔧 TIPO: HTML
📏 LÍNEAS: {len(lines)}
📊 Divs: {div_count} | Semánticas: {semantic_count}
...
"""
        ...
        if not issues and not warnings:
            SESSION_STATE["step_8"] = True
            response += "✅ HTML APROBADO\n\n"
            response += "El HTML cumple con semántica, accesibilidad y la filosofía modular.\n\n"
            response += """➡️ PASO 9 (OBLIGATORIO): Usa philosophy_q9_documentar
...
"""
            ...
            if usuario_confirmo_warnings:
                SESSION_STATE["step_8"] = True
                response += "✅ HTML APROBADO (usuario confirmó ignorar advertencias)\n\n"
                response += """➡️ PASO 9 (OBLIGATORIO): Usa philosophy_q9_documentar
...
"""
                ...
                response += """⚠️ HTML CON ADVERTENCIAS - REQUIERE DECISIÓN DEL USUARIO
...
"""
    ...
    is_complete_file = bool(re.search(
        r'^(<!DOCTYPE|<html|@import|@charset|:root|/\*|import\s|export\s|const\s|let\s|function\s|class\s|"use strict"|\'use strict\')',
...
"""
    ...
    # Validar web: detectar problemas específicos de JS
    if language == "web" and (filename.endswith('.js') or filename.endswith('.mjs')):
        # var en vez de const/let
        var_usage = len(re.findall(r'^var\s+\w+', code, re.MULTILINE))
        if var_usage > 0:
            warnings.append(f"⚠️ {var_usage} usos de 'var'. Usa 'const' o 'let' en su lugar.")

        # DOM queries repetidas
        queries = re.findall(r'document\.(querySelector|getElementById|getElementsByClassName)\s*\([^)]+\)', code)
        query_counts = {}
        for q in queries:
            query_counts[q] = query_counts.get(q, 0) + 1
        repeated_queries = {k: v for k, v in query_counts.items() if v >= 3}
        if repeated_queries:
            warnings.append(f"⚠️ {len(repeated_queries)} queries al DOM repetidas 3+ veces. Guarda en variable.")

        # Inline event handlers en strings de HTML
        if re.search(r'on\w+\s*=\s*["\']', code):
            warnings.append("⚠️ Event handlers inline detectados. Usa addEventListener.")
...
📐 ARQUITECTURA (5 niveles = Atomic Design):
   ...
   PAGE (instancia con datos: pages/*)
        └── TEMPLATE (layout de página: templates/*)
              └── ORGANISM (sección compleja: organisms/*)
                    └── MOLECULE (grupo funcional: molecules/*)
                          └── ATOM (elemento básico: atoms/*)

   Organism = sección reutilizable (header, footer, card grid)
   Template = layout de página (distribución de organismos)
...
def extract_function_signatures(content: str, language: str) -> list:
    ...
    # JS: function nombre(params) o export function nombre(params) o async
    pattern = r'(?:export\s+)?(?:async\s+)?function\s+([a-zA-Z][a-zA-Z0-9_]*)\s*\(([^)]*)\)'
    ...
    # Arrow functions: const nombre = (params) => o const nombre = async (params) =>
    pattern2 = r'(?:export\s+)?const\s+([a-zA-Z][a-zA-Z0-9_]*)\s*=\s*(?:async\s+)?\(([^)]*)\)\s*=>'
...
def get_file_info(file_path: Path, language: str) -> dict:
    ...
    try:
        ...
        if "/atoms/" in path_str or "/atom/" in path_str:
            ...
            nivel_actual = "molecule"
        elif "/organisms/" in path_str or "/organism/" in path_str:
            nivel_actual = "organism"
        elif "/templates/" in path_str or "/template/" in path_str:
            nivel_actual = "template"
        elif "/pages/" in path_str or "/page/" in path_str:
            nivel_actual = "page"
...
def scan_project_files(project_path: Path, language: str) -> list:
    ...
    extensions = {
        "web": [".html", ".css", ".js"],
        "other": [".html", ".css", ".js", ".ts"]
    }

    exts = extensions.get(language, extensions["other"])

    # Carpetas a ignorar
    ignore_dirs = {".git", "__pycache__", "node_modules", "dist", "build", ".cache", "venv", ".venv"}
